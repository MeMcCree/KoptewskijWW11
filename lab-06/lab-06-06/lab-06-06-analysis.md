...
| **JavaScript** | ❌ Нет | Мутабельность по умолчанию | `const` только запрещает переассигнмент |
...
```javascript
// JavaScript - const только запрещает переассигнмент
const xs = [1, 2, 3]
...
Каждый язык оптимален для определенного набор требований:
...
Автор: Коптевский Владислав Владимирович